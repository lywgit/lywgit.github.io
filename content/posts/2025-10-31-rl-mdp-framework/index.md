...
## 背景
大約兩個多月前，我在[強化式學習雜記]({{< ref "posts/2025-09-10-reinforcement-learning/index.md">}})一文中紀錄了一些感想，同時進行了在小遊戲上訓練 DQN 的 [Asteroids-AI 專案]({{< ref "posts/2025-10-19-rl-asteroids-ai-experiments/index.md"  >}})，**擴展我對機器學習整體的認識，多一種的思考問題的可能角度**。

實驗告一段落之後，我原想從 DQN 文章開始回溯釐清一些疑問，後來發現還是該老老實實的看一下經典課本 Sutton & Barto 的 Reinforcement Learning - An Introduction 更有助於系統性的了解領域的語言（[Sutton 本人有提供全文下載](https://x.com/RichardSSutton/status/1519058345088352257)）。

本文紀錄我理解從 Markov Decision Process 開始到 Deep Q-Network 方法為止相關觀念的學習筆記。
...
隨著時序推演兩者層層交替串連構成樹，描繪出各種可能的演變軌跡，取的不同的最終回報。環境動態函數在描述的也就是轉移過程的機率和所得到的獎勵資訊。

#### 展開一次：狀態價值函數和行動價值函數的串聯
...
#### 同策略與異策略 on-policy & off-policy

基於以樣本學習的方法會衍伸出同策略（on-policy）和異策略（off-policy）的差別。假設我們以當下的策略 \\(\pi\\) 與環境互動再以得到數據改善自己的策略，這就稱為同策略的方法。異策略方法則不限制使用的數據必須由代理人當下策略所產生，可以使用其他行為策略（behavior policy）產出的數據來訓練目標策略（target policy）。異策略方法具有更大的彈性，但也帶來訓練不穩定性的挑戰。
...
當 \\(U_t\\) 是真正價值的不偏估計 \\(\mathbb{E}[U_t|S_t] = v\_\pi(s) \\)，則此更新仍可以保證收斂到局部最佳。但在自舉法的情況下由於所有目標的計算都是基於當下的模型參數 \\(\mathbf{w}_t\\)，並不是真正目標的不偏估計，也就不具有相同的保證。
...
經驗回放方法在 DQN 的穩定訓練中扮演了重要的角色，具體來說我們將代理人與環境互動一步的結果以元組 \\((S_t, A_t, R_{t+1},S_{t+1})\\) 的方式儲存下來作為經驗，在梯度下降的 mini-batch 抽樣時隨機取出作為訓練資料使用。由於 Q-learning 屬於離策略的學習方法，因此本就可以使用其他經驗，又相較於標準的 Q-learning 方法可能使用連續的互動資料做訓練，經驗回放的隨機抽養打斷了訓練資料前後的關聯性，降低了不穩定性。
...
（沒有理解錯的話 \\(\tilde{q} \\) 的權重 \\(\mathbf{w}_t\\) 大部分時候其實應該是被固定在某一個較早時間的版本，因此上式中 \\(\tilde{q} \\) 裡的 \\(\mathbf{w}_t\\) 也許應該用其他的符號來表達比才不會跟 \\(\hat{q} \\) 的參數混淆。）


## 後記

不意外的，Sutton & Barto 一書的內容既廣且深，因此開始沒多久我就提醒自己將這一次的主要目標設定在嘗試串連 MDP 到 DQN 之間的基礎觀念就好，對於沒那麼直接相關的深入討論或是範例就先略讀或跳過，結果需要理解的觀念還是比我原本預想的多（收穫良多～）。其他好些重要或有興趣的主題像是 Policy Gradient、Planning、Alpha Go 和 Alpha Zero 等等或是領域新的發展就留待將來了。

從 AI 發展的角度來看，大概沒有人能預測強化式學習本身有沒有機會像 LLM 一樣經歷某種突破或是爆發。我個人是覺得不論如何這個領域有著豐富的觀念和素材，特別是許多嘗試跟人類思考方式借鏡的方法設計都很有趣，將來應該會持續為機器學習領域提供探索與思考的方向。如果哪一天強化式學習在「環境」的取得和利用面上有什麼重大突破，也許也會打開意想不到的篇章。
